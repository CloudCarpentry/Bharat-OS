...
if ($Run) {
    ...
    elseif ($Arch -eq "riscv64") {
        if ($Payload) {
            if (Test-Path "$BuildDir\fw_payload.elf") {
                $qemuArgs += @("-machine", $Machine, "-bios", "none", "-kernel", "$BuildDir\fw_payload.elf", "-m", "256M", "-no-reboot")
            }
            else {
                $qemuArgs += @("-machine", $Machine, "-bios", "$BuildDir\payload.bin", "-m", "256M", "-no-reboot")
            }
        }
        else {
    ...
    elseif ($Arch -eq "arm64") {
        ...
        if ($BootGui -eq "ON") {
            ...
            $qemuArgs += @("-serial", $stdioBackend, "-serial", "vc", "-vga", "none", "-device", "virtio-gpu-device")
            $qemuArgs += @("-nographic")
        }
    }
