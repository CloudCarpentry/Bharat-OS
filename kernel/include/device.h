...
typedef struct {
  ...
  uint32_t device_id;
  uint32_t window_id;
  uint32_t reserved_alignment; // Alignment padding for 64-bit members
  phys_addr_t phys_base;
  virt_addr_t virt_base;
  uint32_t size_bytes;
  uint32_t irq;
  uint32_t in_use;        // Changed to uint32_t for alignment
  uint32_t reserved_tail; // Pad to 48 bytes
...
typedef struct {
  ...
  uint16_t reserved_alignment; // Alignment padding for 64-bit pointers
  const char *compatible;
  uint32_t irq;
  uint32_t hotpluggable; // Changed to uint32_t
  device_power_state_t power_state;
  uint8_t rx_queue_count;
  uint8_t tx_queue_count;
  uint16_t reserved_alignment2; // Pad to align next 32-bit field
  uint32_t security_flags;
  uint32_t perf_flags;
  uint32_t hw_feature_flags;
  uint32_t in_use; // Changed to uint32_t
} device_desc_t;
...
int device_lookup_mmio_window(device_class_t class_id, uint32_t device_id,
                              uint32_t window_id,
...
int device_set_power_state(device_bus_t bus, uint32_t device_id,
