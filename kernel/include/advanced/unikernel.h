 ...
 * Optional single-address-space/library-OS deployment mode for statically linked 
 * trusted workloads (Cloud-Native microservices).
 */

// In Unikernel mode, we run a single trusted execution context
#ifdef BHARAT_UNIKERNEL_BUILD

// The user application must define this entry point for the microkernel to jump to
