...
typedef struct hal_iommu_ops {
    ...
    int (*group_attach)(hal_iommu_group_t *group, hal_iommu_domain_t *domain);
    int (*group_detach)(hal_iommu_group_t *group);
    int (*map)(hal_iommu_domain_t *domain, uint64_t iova, uint64_t phys, size_t size, uint64_t prot);
