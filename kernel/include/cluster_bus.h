 ...
 * acting over CXL or NVLink as a single massively-parallel compute node for AI workload execution.
 */
...
// Map remote node accelerator memory directly into the local physical memory map (CXL.mem)
int cluster_map_remote_memory(cluster_node_t* node);

// Send AI/Compute kernels directly via CXL.cache
int cluster_dispatch_kernel(cluster_node_t* node, gpu_device_t* target, void* kernel_buf, uint32_t size);
