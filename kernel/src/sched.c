...
typedef struct {
  // spinlock_t lock; // Omitting explicit spinlock implementation for
  // simplicity, assuming IRQ disable suffices in UP or atomic ops in SMP
...
void sched_init(void) {
  ...
  for (size_t i = 0; i < BHARAT_ARRAY_SIZE(g_threads); ++i) {
    ...
    g_threads[i].next_free =
        (i + 1 < BHARAT_ARRAY_SIZE(g_threads)) ? (uint32_t)(i + 1) : UINT32_MAX;
  }
  g_free_process_head = 0;
  for (size_t i = 0; i < BHARAT_ARRAY_SIZE(g_processes); ++i) {
    g_processes[i].in_use = 0U;
    g_processes[i].next_free = (i + 1 < BHARAT_ARRAY_SIZE(g_processes))
                                   ? (uint32_t)(i + 1)
                                   : UINT32_MAX;
  }
...
kthread_t *thread_create(kprocess_t *parent, void (*entry_point)(void)) {
  ...
  if (slot) {
    ...
    // Initialize AI scheduler context
    ...
    // Do not add to runqueue directly here since runqueues might not be
    // initialized yet if this is the idle thread being created in sched_init.
    // It's the caller's responsibility or handled via explicit enqueue.
    if (entry_point != (void (*)(void))sched_idle_task &&
        slot->thread.priority < MAX_PRIORITY_LEVELS) {
      list_add(&slot->list_node,
               &g_runqueues[core].ready_queue[slot->thread.priority]);
    }
...
static int sched_suggestion_dequeue(ai_suggestion_t *out) {
  ...
  *out = g_pending_suggestions.queue[g_pending_suggestions.tail];
  g_pending_suggestions.tail =
      (g_pending_suggestions.tail + 1U) % SCHED_MAX_PENDING_SUGGESTIONS;
  return 0;
}
...
static void sched_update_telemetry(kthread_t *thread) {
  ...
  ai_sched_collect_sample(
      thread->ai_sched_ctx, thread->time_slice_ms, thread->cpu_time_consumed,
      sched_run_queue_depth(core_id), (uint32_t)thread->context_switch_count);
}
...
static void sched_switch_to(kthread_t *next, uint32_t core_id) {
  ...
  if (current && current->state == THREAD_STATE_RUNNING &&
      current != g_runqueues[core_id].idle_thread) {
    current->state = THREAD_STATE_READY;
    thread_slot_t *slot = sched_find_thread_slot_by_tid(current->thread_id);
    if (slot) {
      list_add(&slot->list_node,
               &g_runqueues[core_id].ready_queue[current->priority]);
    }
...
void sched_yield(void) {
  ...
  if (g_runqueues[core_id].throttled != 0U &&
      g_runqueues[core_id].idle_thread) {
...
void sched_wakeup_with_priority(kthread_t *thread, uint32_t wakeup_priority) {
  ...
  if (wakeup_priority <= SCHED_MAX_PRIORITY &&
      wakeup_priority > thread->priority) {
  ...
  if (thread->state == THREAD_STATE_SLEEPING ||
    ...
    if (slot) {
      ...
      list_add(&slot->list_node,
               &g_runqueues[core].ready_queue[thread->priority]);
    }
  }
}
...
void sched_on_timer_tick(void) {
  ...
  for (size_t i = 0; i < BHARAT_ARRAY_SIZE(g_threads); ++i) {
    if (g_threads[i].in_use != 0U &&
        g_threads[i].thread.state == THREAD_STATE_SLEEPING &&
...
int sched_sys_thread_create(kprocess_t *parent, void (*entry_point)(void),
                            uint64_t *out_tid) {
...
int sched_migrate_task(kthread_t *thread, uint32_t new_node) {
  if (!thread)
    return -1;
  if (new_node >= MAX_SUPPORTED_CORES)
    return -2;
  ...
  if (thread->state == THREAD_STATE_READY) {
    list_add(&slot->list_node,
             &g_runqueues[new_node].ready_queue[thread->priority]);
  }

  uint32_t current_core = hal_cpu_get_id();
  if (current_core < MAX_SUPPORTED_CORES &&
      g_runqueues[current_core].current_thread == thread) {
...
int sched_enqueue_ai_suggestion(const ai_suggestion_t *suggestion) {
  if (!suggestion)
    return -1;
  uint32_t next =
      (g_pending_suggestions.head + 1U) % SCHED_MAX_PENDING_SUGGESTIONS;
