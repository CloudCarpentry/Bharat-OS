...
int mm_pmm_init(void* memory_map, uint32_t map_size) {
    ...
    // First pass: find available memory bounds and map them to NUMA nodes
        ...
        if (tag->type == MULTIBOOT_TAG_TYPE_MMAP) {
            ...
            for (uint32_t i = 0; i < num_entries; i++) {
                ...
                if (entry->type == MULTIBOOT_MEMORY_AVAILABLE) {
                    ...
                    if (region_start < region_end && active_numa_nodes < MAX_NUMA_NODES) {
                        uint32_t node_id = active_numa_nodes++;
                        uint64_t total_pages = (region_end - region_start) / PAGE_SIZE;

                        numa_nodes[node_id].node_id = node_id;
                        numa_nodes[node_id].start_addr = region_start;
                        numa_nodes[node_id].total_pages = total_pages;
                        ...
                        global_pages_ptrs[node_id] = (page_t*)early_alloc(page_array_size, PAGE_SIZE);

                        if (!global_pages_ptrs[node_id]) {
                            return -1;
                        }
    ...
    if (active_numa_nodes == 0) return -1; // No memory found

    phys_addr_t early_mem_end = early_alloc_get_current_ptr();
    early_mem_end = (early_mem_end + PAGE_SIZE - 1) & ~(PAGE_SIZE - 1);
...
#else
    // Fallback for non x86 architectures... Not fully implemented in original code anyway.
    return -1;
#endif
