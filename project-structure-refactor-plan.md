...
| D1 | `boot/` to `core/boot/` | 🚧 In progress | D1a + D1b completed: sources/common/include/discovery/protocols are now canonical in `core/boot/*`; legacy `boot/*` wrappers remain for compatibility. D1c completed: kernel sub-target CMake include wiring now uses migration-aware `BHARAT_BOOT_INCLUDE_DIR` instead of hardcoded `boot/include` paths. |
