...
void hal_mm_get_zone_limits(hal_mm_zone_limits_t *out) {
    if (!out) return;
    out->dma_low_start = 0;
    out->dma_low_end = 0xFFFFFF; // 16MB limit for old ISA DMA
    out->dma32_start = 0x1000000;
    out->dma32_end = 0xFFFFFFFF; // 4GB limit for 32-bit DMA
...
void hal_mm_backend_caps(hal_mm_backend_caps_t *out) {
    ...
    out->max_regions = 0;
    out->flags = 1; // NX/Sparse mappings supported
}
