...
static bool pl011_init(uart_device_t *dev) {
    ...
    // Keep PL011 enabled with TX enabled (0x101) matching boot.S
    mmio_write32(dev->base + PL011_CR, PL011_CR_UARTEN | PL011_CR_TXE);
    return true;
}

static bool pl011_tx_ready(uart_device_t *dev) {
    if (!dev || !dev->base) return true;
...
static void pl011_flush(uart_device_t *dev) {
    ...
    while (!pl011_tx_ready(dev) && --timeout > 0) {
        // Spin with timeout safety
    }
}
