...
static void test_device_access(void) {
    ...
    assert(cap_table_grant(t, CAP_TYPE_ENDPOINT, 1U, CAP_RIGHT_ENDPOINT_RECEIVE, &cap) == 0);

    // Check if cap is accessible as device
    capability_entry_t resolved;
    int err = cap_table_lookup(t, cap, CAP_TYPE_ENDPOINT, CAP_RIGHT_ENDPOINT_RECEIVE, &resolved);
    assert(err == 0);
