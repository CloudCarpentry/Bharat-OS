...
To handle Android semantics natively, the core kernel process model (`core/kernel/include/sched.h`) has been updated to include a `personality` field in `bh_process_t` and `bh_thread_t`.
