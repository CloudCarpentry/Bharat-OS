...
3. Early Boot Layers: `kernel/src/arch/<arch>/platform` isolates platform-specific assumptions (like QEMU UART locations) from generic boot paths.
4. Framebuffer Fixes: Identity mapping has been dropped. Video metadata is structurally checked against bounds (no overflow, no overlap with kernel/reserved/module memory), and mapped explicitly after VMM initialization (`boot_video_map`).
