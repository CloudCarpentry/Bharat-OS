...
* **`services/` = Policy and Orchestration**
  ...
  * *Must not* contain real hardware driver logic. Hardware control goes in `drivers/`.
...
* **`platform/` = Board/Machine Composition**
...
* ❌ `kernel/src/qemu_hacks.c` (Emulator/board hacks in kernel)
* ❌ `services/netmgr/intel_e1000.c` (Hardware driver inside a service)
* ❌ `kernel/src/fs_policy.c` (Policy logic inside kernel)
