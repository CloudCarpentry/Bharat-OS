...
## Overview
The Backend Telemetry Model (`interface/idl/monitor/accel_telemetry.idl`) tracks the performance, health, and reliability of advanced hardware backends. By accumulating counters for usage and fallbacks, `core/services/accelmgr/` and system administrators can measure exactly how often hardware is utilized versus falling back to software due to contention or failure.
...
2. **Aggregators (`core/services/accelmgr/`)**: Gather these events into rolling health scores.
3. **Deciders (`core/services/powermgr/` or `faultmgr`)**: Trigger safe mode if faults exceed a predefined threshold.
