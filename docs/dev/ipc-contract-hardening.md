...
- Contract types remain in `include/bharat/interface/uapi/ipc/`.
- User-facing API remains in `lib/ipc`.
- Kernel endpoint mechanism remains in `core/kernel/src/ipc/endpoint_ipc.c`.
...
|---|---|---|---|---|
| `include/bharat/interface/uapi/ipc/contract.h` | `bharat_ipc_contract_header_t` | uapi | active, shared in contracts/validation | **Extend** (add `header_version`, `status` only). |
| `include/bharat/interface/uapi/ipc/status.h` | IPC status aliases | uapi | active | **Reuse + extend aliases** (timeout/busy/endpoint). |
| `core/kernel/include/ipc_endpoint.h` | `ipc_endpoint_*`, `ipc_status_t` | kernel | active, core endpoint mechanism | **Reuse** (no redesign). |
| `core/kernel/src/ipc/endpoint_ipc.c` | `ipc_endpoint_create/send/receive` | kernel | active, capability-checked | **Reuse** (lib wraps syscall bridge). |
...
| `core/kernel/src/lib/ds/urpc_ring.c` | ring mechanism | transport/kernel | active | **Reuse** unchanged. |
| `core/kernel/include/bharat/urpc.h` + `core/kernel/src/urpc/*` | channel/bootstrap control | kernel transport | active/partial | **Reuse** unchanged (out of scope redesign). |
