...
### 2. `hal/` (Contracts)
- **Role:** The abstraction layer translating standard OS expectations into `arch/` or `platform/` calls.
...
### 3. `platform/` (SoC/Board Discovery)
...
- **Forbidden:** Directly manipulating MMIO (must go through drivers/kernel).

### 7. `services/` (Policy and Orchestration)
...
- [ ] Is the feature dynamically discovered and populated via `platform/`?
...
- [ ] Is global system policy handled by a dedicated service in `services/` rather than the kernel?
